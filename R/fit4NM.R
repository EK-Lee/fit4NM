...
{  options(guiToolkit="RGtk2")
   ...
   {  dir.create("c:/fit4NM",showWarnings=FALSE)
      ...
      flag.PSEXE<-sum(dir.list=="path.PSEXE")
...
{      
   ...
   {  L1<-svalue(VarList.g1)
      ...
      {  n.temp<-length(Quantile.tot$TIME)      
         ...
         Y.temp2<-c(Quantile.tot$L.97.5,Quantile.tot$U.97.5[n.temp:1])
         polygon(X.temp,Y.temp,col="grey",border="grey")
      ...
      {  n.temp<-length(Quantile.tot$TIME)      
         ...
         Y.temp2<-c(Quantile.tot$L.95,Quantile.tot$U.95[n.temp:1])
         polygon(X.temp,Y.temp,col="grey",border="grey")
         polygon(X.temp,Y.temp1,col="grey60",border="grey")
         polygon(X.temp,Y.temp2,col="grey60",border="grey")
      ...
      {  n.temp<-length(Quantile.tot$TIME)      
         ...
         Y.temp2<-c(Quantile.tot$L.90,Quantile.tot$U.90[n.temp:1])
         polygon(X.temp,Y.temp,col="grey",border="grey")
         polygon(X.temp,Y.temp1,col="grey60",border="grey")
         polygon(X.temp,Y.temp2,col="grey60",border="grey")
   ...
   {  VPC.N<-as.numeric(svalue(N.g1))
      ...
      {  b<-round(i/length(TIME.id)*100)
         ...
         Q97.5<-sort.DV[round(n.DV*0.975)]
         N<-length(ID.list)
         PP<-c(0.025,0.05,0.1,0.9,0.95,0.975)
         L.i<-round(0.5+n.DV*PP-1.96*sqrt(n.DV*PP*(1-PP)))
         L.i<-ifelse(L.i<1,1,L.i); L.i<-ifelse(L.i>n.DV,n.DV,L.i)
         U.i<-round(0.5+n.DV*PP+1.96*sqrt(n.DV*PP*(1-PP)))
         U.i<-ifelse(U.i<1,1,U.i); U.i<-ifelse(U.i>n.DV,n.DV,U.i)         
         Q.L<-sort.DV[L.i]
         Q.U<-sort.DV[U.i]
   ...
   {  VPC.dir<<-gfile("Select runnumber foler with PC.sim",type="selectdir")
      ...
      tmp<-gframe("red solid lines : PI",container=group)      
...
#      temp.CTL[id[1]+1]<-"$ESTIMATION METHOD=SAEM INTER print=1"
   ...
   {   MC<<-as.numeric(svalue(BAYES.input))
      ...
      add(tmp,Boot.label)
      add(tmp,BAYES.input)    
      Boot.label1<-glabel("NBURN")
      BAYES.input1<-gedit("2000",width=5)
      add(tmp,Boot.label1)
      add(tmp,BAYES.input1)      
      Boot.label2<-glabel("NITER")
